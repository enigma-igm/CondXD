...
class CondXD(object):
    ...
    Parameters:
        ...
        hyper_params (dictionary): Hyperparameters used to train the neural network.
        low_SN_mag (float, optional): Reference magnitude above which the covariance matrix is simply diagonal.

    Attributes:
        lr (float): Learning rate.
        batch_size (int): Batch size.
        schedule_factor (float): Factor for reducing learning rate on plateau.
        patience (int): Number of epochs with no improvement after which learning rate will be reduced.
        num_epoch (int): Number of training epochs.
        weight_decay (float): Weight decay (L2 penalty) for regularization.
        size_train (int): Percentage of data used for training.
        size_val (int): Percentage of data used for validation.
        size_test (int): Percentage of data used for testing.
        n_gauss (int): Number of Gaussian components in the mixture model.
        conditional_dim (int): Dimension of the conditional input.

    Methods:
        _get_noise_covar(flux_err, ref_f, low_SN_mag=21):
            Generates the covariance matrix of the relative fluxes.

        _real_size():
            Defines the real size of the training, validation, and testing samples.

        sample_splitting(sample='training'):
            Creates the sample corresponding to the specified parameter: training, validation, or testing.

        gmm_fit(ref_f_train, data_train, rel_err_train, ref_f_val, data_val, rel_err_val, model_name):
            Deconvolves and fits GMM to the training data.
    """
